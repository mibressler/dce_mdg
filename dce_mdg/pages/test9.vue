...
const possiblevalues_time = [
  attributes.time.levels.min20,
  attributes.time.levels.min35,
  attributes.time.levels.min50
];

const possiblevalues_experience = [
  attributes.experience.levels.veryEasy,
  attributes.experience.levels.manageable,
  attributes.experience.levels.cumbersome
];

const possiblevalues_datashared = [
  attributes.datashared.levels.realtime,
  attributes.datashared.levels.anonymized,
  attributes.datashared.levels.none
];
...
const generateCombinations = (values1, values2, values3) => {
  const combinations = [];
...
// Filter forbidden pairs
const filterForbiddenPairs = (combinations, forbiddenPairs) => {
  return combinations.filter(([time, experience, datashared]) => {
    return !forbiddenPairs.some(pair => 
      (pair.alternative0 === time && pair.alternative1 === experience) ||
      (pair.alternative0 === experience && pair.alternative1 === datashared) ||
      (pair.alternative0 === time && pair.alternative1 === datashared)
    );
  });
};

const allCombinations = generateCombinations(possiblevalues_time, possiblevalues_experience, possiblevalues_datashared);
const validCombinations = filterForbiddenPairs(allCombinations, forbidden_level_pairs);

const currentIndex = ref(0);

const currentTable = computed(() => {
  const [time, experience, datashared] = validCombinations[currentIndex.value];
  return [{
    ...
    alternative0: experience,
    alternative1: experience,
  }, {
    attribute: attributes.datashared.label,
    alternative0: datashared,
    alternative1: datashared,
  }];
});

const nextTable = () => {
  currentIndex.value = (currentIndex.value + 1) % validCombinations.length;
};

</script>

<template>
  <UTable :rows="currentTable" :columns="columns">
  ...
  <div class="flex justify-between items-center mt-4">
    <span>Table {{ currentIndex.value + 1 }} of {{ validCombinations.length }}</span>
    <button @click="nextTable" class="btn btn-primary">Next Table</button>
  </div>
</template>
