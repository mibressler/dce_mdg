...
const items = [{
  ...
  alternative1: attributes.datashared.levels.anonymized
}];
...
const computeCombinations = () => {
  let combinations = [];
  possiblevalues_time.forEach(time => {
    possiblevalues_experience.forEach(experience => {
      possiblevalues_datashared.forEach(datashared => {
        if (!isForbidden(time, time, 'time') &&
            !isForbidden(experience, experience, 'experience') &&
            !isForbidden(datashared, datashared, 'datashared')) {
          combinations.push({ time, experience, datashared });
        }
      });
    });
  });
...
// Adjusted isForbidden function that checks per attribute
const isForbidden = (value1, value2, attribute) => {
  return forbidden_level_pairs.some(pair => {
    return pair.attribute === attribute &&
           pair.alternative0.value === value1.value &&
           pair.alternative1.value === value2.value;
  });
};
...
onMounted(() => {
  combinations.value = computeCombinations();
});
